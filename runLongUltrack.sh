#!/bin/bash
#
#SBATCH --job-name=UltrackRun         # Job name
...
#SBATCH --mem=32G                      # RAM for job
#SBATCH --time=06:00:00                # Time limit hh:mm:ss 
#SBATCH --mail-type=END,FAIL           # Email notifications
#SBATCH --mail-user=jakob.schauser@nbi.ku.dk   # Where to send email

echo "=== Job started on $(date) ==="
echo "Running on node: $(hostname)"

# ---- Load environment ----
ENV="ultrack"  # <-- change this to your env name if needed
source /opt/miniconda3/etc/profile.d/conda.sh
conda activate $ENV

# ---- Choose script to run ----
# Set either a Python file OR Notebook file below:
PYTHON_SCRIPT="ultrackpy.py"        # e.g. scripts/train.py
